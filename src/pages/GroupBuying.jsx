...
import { syncGroupBuyToGoogle, deleteGroupBuyFromGoogle } from "@/lib/groupBuyCalendarSync";
import GroupBuyCalendar from "@/components/groupbuy/GroupBuyCalendar";
...
export default function GroupBuying() {
  ...
  const save = useMutation({
    mutationFn: async (data) => {
      const saved = editing
        ? await base44.entities.GroupBuy.update(editing.id, data)
        : await base44.entities.GroupBuy.create(data);
      // 마감일을 구글 캘린더에 등록/수정 (하루 전 알림 포함)
      if (data.end_date) await syncGroupBuyToGoogle(saved?.id || editing?.id);
      return saved;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["groupBuys"] }); setOpen(false); setEditing(null); },
  });
  const remove = useMutation({
    mutationFn: async (item) => {
      await deleteGroupBuyFromGoogle(item.google_event_id);
      return base44.entities.GroupBuy.delete(item.id);
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["groupBuys"] }),
  });
  ...
  return <div className="p-5 md:p-8 space-y-6 max-w-7xl mx-auto">
    ...
    <Tabs defaultValue="list">
      <TabsList><TabsTrigger value="list">목록</TabsTrigger><TabsTrigger value="calendar">일정 캘린더</TabsTrigger></TabsList>
      <TabsContent value="list" className="mt-4">
        <div className="grid lg:grid-cols-[1fr_320px] gap-6"><div>{isLoading ? <div className="py-16 text-center text-muted-foreground">불러오는 중...</div> : <GroupBuyTable items={items} onEdit={edit} onDelete={(item) => { if (window.confirm(`'${item.name}' 공동구매를 삭제할까요?`)) remove.mutate(item); }} />}</div><MarginPreview item={preview} /></div>
      </TabsContent>
      <TabsContent value="calendar" className="mt-4">
        <GroupBuyCalendar items={items} onSelect={edit} />
      </TabsContent>
    </Tabs>
