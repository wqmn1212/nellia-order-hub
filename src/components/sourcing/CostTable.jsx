...
const HEADERS = [
  ...
  { label: "용달비(₩)" },
  { label: "샘플비(₩)" },
  { label: "추가비용(₩)", accent: true },
  { label: "광고비(₩)", accent: true, right: true },
  { label: "개당 수입원가(₩)", accent: true, right: true },
  { label: "입항(ETA)" },
  { label: "" },
];
      ...
      <table className="w-full border-collapse text-xs">
        <thead>
          <tr className="bg-secondary/60 border-b border-border">
            {HEADERS.map((col, i) => (
              <th key={i}
                className={`px-2 py-2.5 font-semibold whitespace-nowrap border-r border-border/60 ${
